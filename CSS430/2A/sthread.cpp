...
/*captures the current stack pointer and base pointer, calculates the size of the stack, 
allocates memory for a copy of the stack, and then copies the contents of the stack 
into that memory. It then pushes the current task control block (cur_tcb) onto a queue 
called thr_queue.
*/
#define capture( ) {							\
...
/*The function first checks if a variable called "alarmed" is true. 
If it is, the function sets "alarmed" to false and then uses 
setjmp and longjmp to save and restore the current thread's 
execution environment (saved in "cur_tcb->env"). 
The captured information is saved in cur_tcb->sp and cur_tcb->stack.
*/    
